...
class ETTSInferenceModel:
    def __init__(self, text_embed_dim, emo_embed_dim, nmels, maxlength,
        ...
        sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'waveglow'))  # For waveglow vocoder
    ...
    def synthesize_with_sample(self, g_emo, text, outputname):
        ...
        with torch.no_grad():
            ...
            g_emo = self.emo_model(g_emo)
            l_emo = torch.softmax(torch.randn(random.randint(80, 160), self.nlst, device=torch.device('cuda:0')) * 10, -1) * 0.25
